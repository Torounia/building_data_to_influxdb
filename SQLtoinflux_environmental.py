...
cnxn = pypyodbc.connect("Driver={ODBC Driver 17 for SQL Server};"
                        ...
                        "Database=HBT.ECD.Awair;"
                        "uid=SQLadmin;pwd=LL22@@")

cursor = cnxn.cursor()
cursor.execute("SELECT * FROM [HBT.ECD.Awair].[dbo].[HBT18019000_Data] where [DPTimeStamp] >" + ((datetime.now() - timedelta(hours=25)).strftime('%y%m%d%H%M%S')) + " order by [DPTimeStamp] desc")
...
datapoint_value = [] #dpvalue datapoint value for the rest
for row in rows:
...
for row in range(len(datapoint_time)):
    influx_points.append(
        ...
        "fields": {
            "value": float(datapoint_value[row]),
            }
    })
...
#TODO
'''
Done - rename measurements to occupancy and environmental from building systems
create logging system
done - create scheduler every 24 hours
find a way to export to CSV
